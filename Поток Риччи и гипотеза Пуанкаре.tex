\documentclass{article}
\usepackage{amsthm}
\usepackage{amssymb}
\usepackage{amsmath}
\usepackage{graphicx}
\usepackage[labelfont=normalfont, labelsep=period]{caption}
\usepackage[utf8]{inputenc} % Кодировка UTF-8
\usepackage[T2A]{fontenc}   % Кодировка для кириллицы
\usepackage[russian]{babel} % Поддержка русского языка
\usepackage[colorlinks=true,linkcolor=blue]{hyperref}
\usepackage{tocloft} % Для настройки оглавления
\usepackage{footnotehyper}

% Убираем точки для всех уровней содержания
\renewcommand{\cftsecleader}{\cftdotfill{\cftnodots}} % Для разделов
\renewcommand{\cftsubsecleader}{\cftdotfill{\cftnodots}} % Для подразделов (если есть)

% Настраиваем нумерацию в содержании
\renewcommand{\thesubsection}{\arabic{subsection}}
\renewcommand{\thesection}{Глава \arabic{section}.}
\renewcommand{\cftsecaftersnum}{ }
\cftsetindents{section}{0em}{5.0em}

% Настройка вида блоков

% Или для другого шрифта, например, шрифт без засечек:
\newtheoremstyle{theoremstyle}
{10pt}         % Вершина
{10pt}         % Низ
{\normalfont}  % Обычный шрифт для текста заголовка
{}             % Отступ
{\scshape}     % Small Caps для заголовка
{.}            % Печать
{10pt}          % Разделитель
{}             % Стиль

\theoremstyle{theoremstyle}
\newtheorem{theorem}{Теорема}[section] % Нумерация Теорем по главам
\renewcommand{\thetheorem}{\arabic{section}.\arabic{theorem}}

\theoremstyle{theoremstyle}
\newtheorem{corollary}[theorem]{Следствие} % Нумерация Следствий по главам
\renewcommand{\thecorollary}{\arabic{section}.\arabic{corollary}}

\theoremstyle{theoremstyle}
\newtheorem{remark}[theorem]{Замечание} % Нумерация Замечаний по главам
\renewcommand{\theremark}{\arabic{section}.\arabic{remark}}

\theoremstyle{theoremstyle}
\newtheorem*{myproof}{Доказательство}


% Настройка для отображения "Рисунок" с маленькими заглавными буквами
\DeclareCaptionLabelFormat{custom}{\textsc{Рисунок}\ \thefigure}
\captionsetup[figure]{labelformat=custom}

\renewcommand{\theequation}{\arabic{section}.\arabic{equation}} %нумерация уравнений по секциям


\begin{document}

\begin{center}
    \huge\textbf{Поток Риччи и гипотеза Пуанкаре}
    \vspace{1cm}

    \huge{Джон В. Морган и Ган Тянь}
\end{center}
\newpage
\newpage
\tableofcontents
\newpage
\input{chapterIntro/chapter.tex}
\newpage
\input{chapter1/chapter.tex}
\newpage
\input{chapter2/chapter.tex}
\newpage
\input{chapter3/chapter.tex}
\newpage
\input{chapter4/chapter.tex}
\newpage
\input{chapter5/chapter.tex}
\newpage
\input{chapter6/chapter.tex}
\newpage
\input{chapter7/chapter.tex}
\newpage
\input{chapter8/chapter.tex}
\newpage
\input{chapter9/chapter.tex}
\newpage
\input{chapter10/chapter.tex}
\newpage
\input{chapter11/chapter.tex}
\newpage
\input{chapter12/chapter.tex}
\newpage
\input{chapter13/chapter.tex}
\newpage
\input{chapter14/chapter.tex}
\newpage
\input{chapter15/chapter.tex}
\newpage
\input{chapter16/chapter.tex}
\newpage
\input{chapter17/chapter.tex}
\newpage
\input{chapter18/chapter.tex}
\newpage
\input{chapter19/chapter.tex}
\newpage
\input{chapterBibliography/chapter.tex}

\end{document}